...
$tags = explode(',', $matchingArticle["tags"]);

foreach($tags as $tag) {
  $p = $dom->createElement("p");

  $p->nodeValue = trim($tag);
...
if($nav) {
  ...
  if($currentIndex > 0) {
    ...
    $prevLink->setAttribute("href", "/~asharma13/csen161/lab5/index.php/article?id=" . $prevArticle["id"]);
  ...
  if($currentIndex < count($articles) - 1) {
    ...
    $nextLink->setAttribute("href", "/~asharma13/csen161/lab5/index.php/article?id=" . $nextArticle["id"]);
